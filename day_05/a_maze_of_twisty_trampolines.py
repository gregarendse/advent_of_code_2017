...
"""
example_input = [0, 3, 0, 1, -3]

def jump_instructions(input) -> int:
    pos = 0
    iterations = 0

    while (pos < len(input)):
        off_set = int(input[pos])
        if off_set >= 3:
            input[pos] = int(input[pos]) - 1
        else:
            input[pos] = int(input[pos]) + 1
        pos = pos + off_set
...
print("Iterations: ", jump_instructions(example_input))

with open('day05/input') as file:
    input = file.readlines()

print('Iterations: ', jump_instructions(input))