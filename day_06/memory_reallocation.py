...
def find_reallocation_repeat(input) -> [int, int]:
    iterations = {}
    count = 0

    while (str(input) not in iterations):
        if iterations.get(str(input)) is None:
            iterations[str(input)] = count
        input = memory_reallocation(input)
        # print(input)
        count = count + 1

    return [count, count - iterations[str(input)]]

def memory_reallocation(input):
    biggest_pos = find_biggest_block(input)
    value = input[biggest_pos]
    input[biggest_pos] = 0

    for i in range(value):
        pos = (i + 1 + biggest_pos) % len(input)
        input[pos] = input[pos] + 1

    return input

def find_biggest_block(input) -> int:
    pos = 0

    for i in range(len(input)):
        if int(input[i]) > int(input[pos]):
            pos = i
...
print(input)
[count, uniqu] = find_reallocation_repeat(input)
print("Count: ", count)
print("Uniqe: ", uniqu)