...
def knot_hash_round(input, lengths, current_position=0, skip_size=0):
    ...
    return [input, current_position % len(lengths), skip_size]
ranges = [int(i) for i in range(256)]
[input, position, size] = knot_hash_round(ranges, [3, 4, 1, 5, 17, 31, 73, 47, 23])
print([input, position, size])
assert size == 4
assert position == 4
...
def knot_hash(input):
    ...
    ranges = [int(i) for i in range(256)]
    for i in range(64):
        [ranges, position, size] = knot_hash_round(ranges, ascii_codes, position, size)
...
[result, current_position, skip_size] = knot_hash_round(input, [3,4,1,5])
print([result, current_position, skip_size])
...
ranges = [int(i) for i in range(256)]
[result, current_position, skip_size] = knot_hash_round(ranges, lengths)
assert (result[0] * result[1]) == 38415
